# 简化版并发测试脚本 - 测试updateHoldings接口的数据一致性
...
try {
    $response = Invoke-WebRequest -Uri "http://localhost:8090/api/holdings/current" -Method GET -UseBasicParsing -ErrorAction Stop
    $currentHoldings = $response.Content | ConvertFrom-Json
    Write-Host "当前持仓数据："
...
$job1 = Start-Job -ScriptBlock {
    ...
    try {
        ...
        $response = Invoke-WebRequest -Uri "http://localhost:8090/api/holdings/update" -Method POST -ContentType "application/json" -Body $jsonBody -UseBasicParsing
        return @{TaskId=1; Status="Success"; StatusCode=$response.StatusCode; Content=$response.Content}
    } catch {
        return @{TaskId=1; Status="Failed"; ErrorMessage=$_.ToString()}
    }
} -ArgumentList @($request1)
...
$job2 = Start-Job -ScriptBlock {
    ...
    try {
        $jsonBody = $requestBody | ConvertTo-Json -Depth 3
        $response = Invoke-WebRequest -Uri "http://localhost:8090/api/holdings/update" -Method POST -ContentType "application/json" -Body $jsonBody -UseBasicParsing
        return @{TaskId=2; Status="Success"; StatusCode=$response.StatusCode; Content=$response.Content}
    } catch {
        return @{TaskId=2; Status="Failed"; ErrorMessage=$_.ToString()}
    }
} -ArgumentList @($request2)
...
$job3 = Start-Job -ScriptBlock {
    ...
    try {
        $jsonBody = $requestBody | ConvertTo-Json -Depth 3
        $response = Invoke-WebRequest -Uri "http://localhost:8090/api/holdings/update" -Method POST -ContentType "application/json" -Body $jsonBody -UseBasicParsing
        return @{TaskId=3; Status="Success"; StatusCode=$response.StatusCode; Content=$response.Content}
    } catch {
        return @{TaskId=3; Status="Failed"; ErrorMessage=$_.ToString()}
    }
} -ArgumentList @($request3)

# 等待所有任务完成并获取结果
Write-Host "\n等待所有并发任务完成..."
$results = @()
$results += Wait-Job -Job $job1 -Timeout 30 | Receive-Job
$results += Wait-Job -Job $job2 -Timeout 30 | Receive-Job
$results += Wait-Job -Job $job3 -Timeout 30 | Receive-Job
...
foreach ($result in $results) {
    if ($result -and $result.Status -eq "Success") {
        ...
        Write-Host "任务未返回结果或超时" -ForegroundColor Red
    }
}
...
try {
    $response = Invoke-WebRequest -Uri "http://localhost:8090/api/holdings/current" -Method GET -UseBasicParsing -ErrorAction Stop
    $finalHoldings = $response.Content | ConvertFrom-Json
    Write-Host "最终持仓数据："
...
if ($finalHoldings -and $finalHoldings.Count -gt 0) {
    ...
    Write-Host "\n❌ 无法获取有效的最终持仓数据进行分析" -ForegroundColor Red
}
