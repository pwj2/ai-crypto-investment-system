...
app.get('/api/holdings/current', async (req, res) => {
  try {
    const [rows] = await pool.execute('SELECT id, coin_type as type, amount as quantity, price, total_value as totalValue FROM crypto_holding WHERE is_current = 1');
    res.json(rows);
  } catch (error) {
...
app.get('/api/holdings/history', async (req, res) => {
  try {
    ...
    // 构建SQL查询和参数
    let sql = 'SELECT id, coin_type, amount, price, total_value, update_time FROM crypto_holding WHERE is_current = 0 AND update_time != \'\'';
    const params = [];
...
app.post('/api/holdings', async (req, res) => {
  ...
  try {
    ...
    // 首先将所有当前持仓标记为非当前
    await connection.execute('UPDATE crypto_holding SET is_current = 0 WHERE is_current = 1');
...
app.get('/api/suggest-reports', async (req, res) => {
  ...
  try {
    // 获取总数
    const [totalResult] = await pool.execute('SELECT COUNT(*) AS total FROM crypto_suggest_report');
    ...
    res.status(500).json({ error: '获取建议报告列表失败', details: error.message });
  }
});
...
app.get('/api/suggest-reports/:id', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: '获取建议报告详情失败', details: error.message });
  }
});
...
app.put('/api/suggest-reports/:id/status', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: '更新建议报告状态失败', details: error.message });
  }
});
...
app.delete('/api/suggest-reports/:id', async (req, res) => {
  ...
  try {
    await pool.execute(
      'DELETE FROM crypto_suggest_report WHERE id = ?',
      [id]
    );
...
app.get('/api/audit-records', async (req, res) => {
  ...
  try {
    // 获取总数
    const [totalResult] = await pool.execute('SELECT COUNT(*) AS total FROM audit_records');
    const total = totalResult[0].total;
